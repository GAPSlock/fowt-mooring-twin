...
\midrule
Linear Regression & 44.4 & 0.675 & 0.001 \\
Random Forest & 35.5 & 0.792 & 8.4 \\
\textbf{Neural Surrogate (Ours)} & \textbf{20.6} & \textbf{0.930} & \textbf{0.2} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:baselines} illustrates this comparison. The failure of Linear Regression ($R^2 = 0.675$) on the unseen sea states quantitatively proves that mooring line dynamics are fundamentally non-linear and require deep representational capacity to generalize. While Random Forest models capture some of this non-linearity ($R^2 = 0.792$), their sequential tree-traversal inference time is too slow for real-time edge processing, and they struggle to smoothly interpolate extreme kinematics. The neural surrogate, conversely, optimally balances fidelity and speed, maintaining strong generalizability with an $R^2$ of 0.930 and an RMSE of 20.6\,kN, while executing in just 0.2\,ms (5000\,Hz capability, well beyond the 50\,Hz required for real-time tracking). This non-linearity arises from quadratic drag ($\propto v^2$), geometric catenary effects, and coupling between translational and rotational DOFs.
